...
class TestAddCourseViewController: UIViewController {
    
    @IBOutlet weak var viewChangeMain: UIView!
    ...
    override func viewDidLoad() {
        ...
        changeView.translatesAutoresizingMaskIntoConstraints = true
        getData()
        design()
    ...
    private func defaultLineInTextView() {
        colorSelect = .white
        fontSelect = .systemFont(ofSize: 16)
        alignment = .left
        sizeFontSelect = 16
    }
    ...
    func getLinePosition(for characterIndex: Int, in textView: UITextView) -> CGPoint {
        var currentPosition = 0
        var lineNumber = 0
        var lineStart = 0
        for line in textView.text.components(separatedBy: "\n") {
            currentPosition += line.count + 1
            if characterIndex < currentPosition {
                let lineRect = textView.caretRect(for: textView.position(from: textView.beginningOfDocument, offset: lineStart)!)
                return lineRect.origin
            }
            lineNumber += 1
    ...
    func getLineNumber(for characterIndex: Int, in textView: UITextView) -> Int {
        var currentPosition = 0
        var lineNumber = 0
        for line in textView.text.components(separatedBy: "\n") {
            currentPosition += line.count + 1 // +1 для символа новой строки
            if characterIndex < currentPosition {
                return lineNumber
            }
            lineNumber += 1
...
// MARK: - TextView
extension TestAddCourseViewController: UITextViewDelegate {
    
    func textViewDidChangeSelection(_ textView: UITextView) {
        let selectedRange = textView.selectedRange
        
        if selectedRange.length > 0 {
            viewChangeMain.isHidden = false
            changeView.isHidden = true
        }else {
            viewChangeMain.isHidden = true
            let cursorPosition = textView.selectedRange.location
            changePositionView(positionView: getLinePosition(for: cursorPosition, in: textView))
            
            let lineNumber = getLineNumber(for: cursorPosition, in: textView)
            
            let cursorLine = textView.text.components(separatedBy: "\n")[lineNumber]
            
            if cursorLine.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                changeView.isHidden = false
                defaultLineInTextView()
            } else {
                changeView.isHidden = true
            }
        }
    ...
    func changePositionView(positionView: CGPoint) {
        let viewX = positionView.x + changeView.frame.width + 20
        let viewY = positionView.y + textView.frame.minY - 3
        
        changeView.frame = CGRect(x: viewX, y: viewY, width: changeView.frame.width, height: changeView.frame.height)
    }
...
// MARK: - Image
extension TestAddCourseViewController: UIImagePickerControllerDelegate & UINavigationControllerDelegate {
...
// MARK: - Font
extension TestAddCourseViewController: UIFontPickerViewControllerDelegate {
...
// MARK: - Color
extension TestAddCourseViewController: UIColorPickerViewControllerDelegate {
